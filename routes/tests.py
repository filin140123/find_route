...
class AllTestsCase(TestCase):
    ...
    def test_model_train_duplicate2(self):
        """Тестирование возникновения ошибки при создании дубля поезда"""
        train = Train(name='t12345', from_city=self.city_A, to_city=self.city_B, travel_time=9)
        with self.assertRaises(ValidationError):
            train.full_clean()
        try:
            train.full_clean()
    ...
    def test_valid_route_form(self):
        data = {"from_city": self.city_A.id, "to_city": self.city_B.id,
                "cities": [self.city_E.id, self.city_D.id], "traveling_time": 9}
        form = RouteForm(data=data)
        self.assertTrue(form.is_valid())

    def test_valid_route_form2(self):
        data = {"from_city": self.city_A.id, "to_city": self.city_B.id,
